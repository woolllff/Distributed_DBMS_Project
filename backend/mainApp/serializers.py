...
class localSchemaSerializer(serializers.ModelSerializer):
    ...
    def create(self, data):
        ls = LocalSchema(data["serverIP"], data["serverUser"], data["serverPass"], data["dbName"])
...
class globalSchemaSerializer(serializers.ModelSerializer):
    ...
    def create(self, data):
        ...
        for i in json.loads(data["localSchemas"])["ids"]:
            ...
            gs.addLocalSchema(i,ls.serverIP, ls.serverUser, ls.serverPass, ls.dbName)
    ...
    def update(self, instance, validated_data):
        ...
        for i in json.loads(data["localSchemas"])["ids"]:
            ls = localSchemaModel.objects.get(id = i)
            gs.updateLocalSchema(i,ls.serverIP, ls.serverUser, ls.serverPass, ls.dbName)
